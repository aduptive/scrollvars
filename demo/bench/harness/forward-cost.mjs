...
async function once(variant) {
  ...
  try {
    ...
    if (variant === 'forward')
      await page.evaluate(() => {
        // One write, as today. The clocks are registered non-inheriting and a
        // single rule forwards them to the direct children that consume them,
        // so propagation stops there instead of walking every text node.
        const style = document.createElement('style')
        style.textContent = '.sv > * { --sv-t: inherit; --sv-view: inherit }'
