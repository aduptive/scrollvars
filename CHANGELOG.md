...
- Perf phase 1, stop work nobody sees. React `<Slider autoplay>` already skipped a tick while off screen or with the tab hidden; the `setInterval` itself now stops there too and restarts on return, instead of waking up every interval on a background tab full of never-visible carousels. `ownership()` (the journal every inline style/class/attribute write in this library goes through, including the slider's own per-frame `--sd`/`--sv-progress`) now skips the actual DOM write when the value already reads back as what it is about to write; the restore-on-stop bookkeeping still runs every call, so `restore()`'s semantics are unchanged. Both are correctness fixes ("do no work off screen"), not headline wins: measured on the perf lab's `long.html` (marquee and slider both present) the frame cadence is unchanged on this machine, as expected for background work that was already cheap to skip; the actual stopping is what the new tests assert directly (an exact `setInterval`/`clearInterval` count for autoplay, a mocked style write count for the journal).
...
- `clearReleased()` cleared the pending `data-sv-off` marker on retrack but not the pending `--sv-live` lift the fix above added: an element settled through `settleUntracked()` while a nested tracker was still live, then retracked before that descendant released, stayed queued for the lift forever, a harmless (same value, `--sv-live: 1`) but permanent memory retention leak. `clearReleased()` now drops the element itself, never an ancestor, from that set too.
- fx gallery: `CoverflowSlider` wrapped every top-level child in its own `<Slide>`, so a fragment of slides collapsed into one and a child that was already a `<Slide span={2}>` had its `--sv-span` land on an inner div the column rule never sizes by. Fragments are opened recursively now (keyed the same way the kit's own rail does), and an authored `Slide` gets `cf-slide` added to it instead of a second wrapper.
- An outer pinned section that fell back to document flow (its fit box overflowed) released its own `.sv-stage` but left a nested tracker's tall wrapper and static, clipping stage behind: `[data-sv-flow] .sv-stage` (styles/pin.css) releases every descendant stage, not only the outer one's, so the driver now latches flow on every tracked entry nested inside the one that just overflowed too.
- The CLI silently installed from the public registry whenever an explicit `SCROLLVARS_REGISTRY` failed to fetch, which could write a public component under the same slug a private registry meant to own. An override now exits 1 naming the URL and the error instead of falling back.
- `scan()`'s removal handling matched a detached node's CURRENT attributes, so stripping `data-sv`/`data-sv-split`/`data-sv-pointer` before removing the node left its tracker, split or pointer lease registered forever. Removal is now driven by the scan's own lease maps (released whenever the element no longer sits inside the scanned scope), independent of what it carries by the time the observer fires.
- `slider()` read whether an element's effective `scroll-snap-type` was already `none` before adding the `.sv-slider` class that itself declares one, so an unclassed element (any element not already carrying the class before the call) always read a plain `none` and had its custom drag/wheel settle permanently disabled. The read now happens after the class lands.
- `mapRange()` floored a reversed range's span at an epsilon, so every reversed range answered 0 instead of the same ratio `sv-range`'s CSS twin computes: `mapRange(.75, 1, 0)` returned `0`, now `.25`; `mapRange(.25, 1, 0)` returned `0`, now `.75`, matching `styles/pin.css`'s `clamp(0, (t - from) / (to - from), 1)` exactly. It now divides by the real span, and steps at `to` only when `from === to` has no ratio to compute at all.
- `toggles()`'s click handler resolved a trigger's `data-sv-target` unguarded, so one click on a malformed selector (a CMS typo) threw, tore down the whole instance, and froze every toggle in that scope in whatever state it was in. `click()` now tolerates an unparsable selector the same way `boot()` already does: it skips that trigger and keeps running.
- A tracker attached under an ancestor that had already switched to document flow (a React `<Track>` retrack, a CMS block mounted later, a manual retrack) never latched itself: its own stage was already released by `[data-sv-flow] .sv-stage`, so it could never measure an overflow of its own, and its tall pin wrapper sat orphaned under a now-static stage. `attach()` now checks for a flowing ancestor and latches immediately.
- fx gallery: `cube-windows` declared `requires.min: '1.9.0'` while its installed source imports `onMotionChange` (1.17.0), so the CLI told a consumer on an older install "ok" and the import failed at runtime; the gallery preview's inline script read `SV` before the engine bundle that defines it had loaded, throwing `SV is not defined` and silently never mounting the effect; and the React component's effect constructed a `ResizeObserver` unguarded, so a browser below the individual-transform floor without `compat()` threw and unmounted the whole root instead of rendering the content whole. `requires.min` is now `1.17.0`, the preview destructures `SV` inside its `load` handler, and both the vanilla and React mount code return early without a `ResizeObserver` (no clip-path, content whole, same contract as every other fallback path). Recategorized from `Sections` to `Pointer` (it renders through the three-format effect pipeline, not the Section preview pipeline).
- fx gallery: `DeckSpread` and `SequencedScrub` wrapped every top-level child individually, the same bug `CoverflowSlider` had (round-15 fix, untwinned): a fragment of two cards collapsed into one fan slot / one scrub window, and `--sv-mid` centred on a card that was not counted right. Fragments are opened recursively now, keyed the same way the coverflow rail already does.
- `refresh()` and the resize path refreshed a pinned entry's stage origin one entry at a time, each one toggling `position: static`, reading `offsetTop` up the chain, then restoring: N pinned entries forced N style recalculations on any document-height change (lazy images, fonts, a feed), not one. The read now happens for every entry after every entry's static override is in place, restoring only once all reads are done, the same read-all-then-write-all shape the pin helper already uses.
- `slider()` treated an authored `tabindex="-1"` (a follower rail deliberately kept out of the tab order) the same as no `tabindex` at all, since both read `-1` through `.tabIndex`, and overwrote it with `0`. The check now reads the attribute.
- The CLI joined an absolute `--dir` under `process.cwd()` (`join('/a', '/b')` is `/a/b`), writing the installed file somewhere under the current directory instead of the given absolute path. It now resolves the path instead of joining it.
- A `<link rel="stylesheet" disabled>` has no parsed sheet either, the same uncertainty signal as one still loading, but nothing ever fires its `load`/`error`: the driver treated it as pending forever and kept `--sv-page`/`--sv-v` publishing for the whole session even with no other consumer. A disabled link is no longer counted as pending.
- `slider()` on an empty rail fired `onSlide(0)` for a slide that did not exist, so when slides arrived later the real first one never got its own `onSlide(0)` (index 0 already read as "active"). An empty rail no longer claims an index.

### Docs

- Corrected two out-of-date descriptions of the failed-scan and page-outputs behavior above: Boot's watchdog stands down on any successful tracker or scan, not on "completed scanner setup" specifically, and a failed scan now settles its own content visible; and the `--sv-page`/`--sv-v` publishing trigger is CSS mentioning them (a `var()` read, a declaration, even a comment), not only CSS reading them.
- "JavaScript never animates styles" (AGENTS.md, llms.txt) had a shipped counterexample: `cube-windows` writes `el.style.clipPath` from its own rAF loop. Corrected to note the exception (cube-windows, the canvas effects).
- AGENTS.md and `styles/scoped.css` pointed at a README hook-examples section, an Accessibility section and a "Scoped clocks" section that moved to `docs/guide.md` whole; corrected to point at the guide. AGENTS.md's own `style={{'--sv-order': i}}` snippet failed to type-check (an excess property against `React.CSSProperties`); corrected to the cast the library itself uses. Three installed gallery components (`HeroCinematic`, `ThreeScene`, `PointerTiltGrid`) carried a redundant ref cast and a comment claiming a ref hook returns `RefObject<T | null>`; every hook has returned `React.RefObject<T>` since ADU-106, and the casts and the false comment are gone.
...
- The "fully animated" browser floor had SIX hand-typed copies and three of
  ...
  draft in `article/`, which is not published from this repo, all said
  ...
  all three to Firefox 78+. The five copies that ship now render from one
  source; the draft sits outside the generated pipeline and was corrected by
  hand.

### Tooling (ADU-181)
- The browser floor's five shipped copies now render from one
  `BROWSER_FLOOR` in `scripts/docs-data.mjs`, next to `COMPAT_PRESETS`
  (ADU-176's pattern):
