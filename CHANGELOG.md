...
- The stamped `scrollvars/react` gzip size is now measured with esbuild's real code splitting, entry chunk only: the dev-only `?sv-debug` overlay (loaded on demand, never on a page that does not ask for it) is no longer counted toward it. The published number moves from 18.2 KB to 17.3 KB for this reason, a measurement fix, not because the React layer shrank; see `scripts/docs-data.mjs`'s `measureSizes()` for the split build.
