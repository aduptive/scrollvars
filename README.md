...
| --- | --- |
| `track` (the driver) | 2.5 KB |
| `track` + `scan` (zero-wrapper mode) | 3.4 KB |
| `slider` | 2.1 KB |
| `trackPointer` | 0.5 KB |
| `mountEffect` (canvas) | 1.5 KB |
| everything in `scrollvars` (the core entry) | 6.1 KB |
| `scrollvars/react` (wrappers + kit, React external) | 11.5 KB |
<!-- sizes:end -->

A typical page (reveals + stagger) ships `track` + `styles/core.css`:
**~4.9 KB gzipped, total.**
