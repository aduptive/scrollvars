...
// also styles/pin.css (3.1), slider.css (1.3), tilt.css (0.6), state.css (2.2, scroll-driven acts need core too), ui.css (0.9), per page needs
```
...
instead of reading `--sv-duration`/`--sv-stagger`, and a knob declared on
a descendant instead of inherited, which is what `<Item duration>` and
`<Split>` emit; both dip and reverse instead of entering.
...
first paint, nothing to undo. Under a strict CSP with no
