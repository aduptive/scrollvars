...
`translate: calc((1 - var(--sv-pin, 0)) * var(--sv-rail-start, var(--sv-stage-width, 100vw)) + var(--sv-pin, 0) * min(var(--sv-stage-width, 100vw) - 100%, 0px)) 0;`).
...
flavor or consume `--sv-r` yourself (ALWAYS as `var(--sv-r, 1)`; `--sv-r` is a registered property with initial value 1, but only where the engine supports `@property` too (Chrome 85/Safari 16.4/FF 128); below that floor, or below the calc division floor alone (Chrome 112/Safari 16.4/FF 112), the property is unregistered and a consumer's own declaration falls back to its initial value instead of 1; `var(--sv-r, 1)` is habit, not the reason older engines settle at the end state, and never fires on your range children either way, since `--sv-r` is always set; override the clock on the container, `.mine { --sv-clock: var(--sv-t) }` (needs `travel: true` / `data-sv-travel`, or `--sv-t` sits at 0)). JS twin: `mapRange(t, from, to, ease?)` inside
...
any successful tracker or scan acknowledges boot, including on empty routes; a failed scan settles its own content visible instead of waiting on boot at all. Under a strict CSP with no
...
Installed StickySteps starts static, temporarily stacks candidate shots without
...
`TrackOptions.onStatus` is the supported per-lease notification. Import its
...
the first successful measurement, output and output callbacks; `completed`
follows the final visible output of an entrance-only `once` lease, which can
finish without an `active` notification. `released` follows static settlement
on cleanup, replacement or boot release. `failed` follows static settlement
after initialization, measurement, output or callback failure. Each transition
fires once; no per-frame repeats and no notification after `released`.
A completed entrance stays visible and its cleanup is a no-op. React reads
the latest callback without reattaching or replaying status. `track()` keeps
returning its cleanup function.

An active tracker does not prove that a Section's crossfade CSS exists or
that the content fits. StickySteps combines status with its fit and computed
layout checks, then clears `inert` and `aria-hidden` on failure or release.
Reducing motion mid-flight leaves the lease active but restores static,
reachable media; returning to normal motion can resume a working crossfade.
The travel clock keeps scrubbing; pin and scene finish at once as the helper-pinned wrapper returns to flow. An overflow `data-sv-flow` latch
survives both shrinking content and motion reversal until an explicit retrack.
Call `track()` again (or remount the React tracker) after fixing a recoverable
failure. The three-second Boot watchdog is terminal for the page session:
late tracking, scanning and remounts stay static. A later track lease reports
`attaching`, then `failed`.
...
`<Scenes onScene>` is a React notification: a throwing consumer is reported once
and disabled until remount, while scene state and healthy siblings keep working.
...
author changes. Child replacement restores departed slides' owned outputs and
classes on mutation delivery and releases their ownership records.
Keep native rail styling in authored markup (`class="sv-slider"`)
...
`prefers-reduced-motion` is honored by every preset sheet and by the driver,
the slider's glide, canvas effects (`fx.reducedMotion`) and the React
`<Slider>` autoplay. The page's own switch is `html[data-sv-motion="reduce"]`:
every reduced-motion block in the sheets has a twin under it.
`setMotion('reduce' | 'auto')` sets the attribute, `onMotionChange(fn)`
reports the effective preference live, `prefersReducedMotion()` reads it.
Persist the choice yourself and set it before the first frame (an inline
script in `<head>` that reads your store and sets the attribute, before any
stylesheet paints). Your own CSS on the clocks (`--sv-view`, `--sv-t`,
`--mx`, `--my`) is motion too: guard it with the media query and
`:where([data-sv-motion="reduce"])`; in Tailwind, add a variant next to
`motion-reduce:` (`addVariant('sv-reduce', 'html[data-sv-motion="reduce"] &')`)
and use both. The kit's slider
