...
Persist the choice yourself and set it before the first frame. Your own CSS
on the clocks (`--sv-view`, `--sv-t`, `--mx`, `--my`) is motion too: guard it
with the media query and `:where([data-sv-motion="reduce"])`. Slider dots are
24 by 24 CSS pixel targets (`--sv-dot-target`) with the visual dot inside
(`--sv-dot-size`); `split()` takes plain text only. The README's
