...
let html = `<p class="sub">Measured by the committed harness (<a href="https://github.com/aduptive/scrollvars" style="color:#f2c14e"><code>demo/bench/harness</code></a>. Clone the repo,
...
<h2 style="font-size:15px; margin-top:18px;">The style-recalc curve <span style="color:#8fa898; font-weight:400;">(the honest cost of the CSS-variable mechanism)</span></h2>
...
if (existsSync(throttledPath)) {
  ...
  if (main4) {
    html += `
<h2 style="font-size:15px; margin-top:18px;">Low-end profile <span style="color:#8fa898; font-weight:400;">(4× synthetic CPU throttle; not a physical phone)</span></h2>
...
<p class="sub">Raw: <a href="results/throttled-4x.json" style="color:#f2c14e">results/throttled-4x.json</a>.</p>`
  }
}
...
if (app) {
  ...
  html += `
<h2 style="font-size:15px; margin-top:18px;">An app-shaped page <span style="color:#8fa898; font-weight:400;">(what the library adds to a page that lives)</span></h2>
...
<p class="sub">Raw: <a href="results/app-shaped.json" style="color:#f2c14e">results/app-shaped.json</a>.</p>`
}
...
// ── the same numbers in docs/guide.md and AGENTS.md (markdown, between markers) ──
...
for (const file of ['docs/guide.md', 'AGENTS.md']) {
...
console.log(`bench tables stamped (guide, AGENTS, bench page; ScrollVars ${sizes.everything} KB gz, ~${ratio}× vs GSAP)`)
